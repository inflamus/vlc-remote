...
{/*
	...
	this.plModal = this._modal('playlist', '<span class="icon icon-list"></span> Playlists')
				.append(
					$('<footer>', {'class':'bar bar-footer'})
						.append($('<div>', {'class':'segmented-control'})
							.append($('<a>', {'class':'control-item', html:'Add', 'href':'#browse'})
								.click(function(){
// 									that.plModal.parent().parent().removeClass('active');
									that.pl_opener=true;
									$('.modal#browse > header > a.icon-close').click(function(){
										that.reloadPlaylist();
										that.pl_opener = false;
										})
								}))
							.append($('<a>', {'class':'control-item', html:'Clear'})
								.click(function(){
									if(that.status.state != 'stopped')
										if(!confirm('Something is currently playing and will be stopped.'+"\n"+'Would you like to continue ?'))
											return false;
									that.reloadPlaylist(true);
								}))
						 )
				 ).find('.content').css('padding-bottom', '44px')
				.append('<ul class="table-view"></ul>')
				.find('ul.table-view');
					...
					if($.inArray(ext, video_types) != -1 || $.inArray(ext, audio_types) != -1 || $.inArray(ext, playlist_types) != -1 || this.type=='dir')
						...
						.append($('<li class="table-view-cell">')
							.append(
								...
								:	$('<a data-openfile="'+this.uri+'">')
										.append('<span class="media-object pull-left icon icon-play"></span>')
										.append(' '+this.name)
										.append($('<button>', {'class':'btn btn-outlined btn-positive', html:'<span class="icon icon-plus"></span> <span class="icon icon-list"></span>'})
											.click(function(e){
													e.stopPropagation();
													that.sendCommand({'command':'in_enqueue', 'input':$(this).parent().attr('data-openfile')});
// 													that.browseModal.parent().removeClass('active');
													return ;
											}))
										.click(function(){
											that.sendCommand({'command': (that.pl_opener ? 'in_enqueue' : 'in_play'), 'input':$(this).attr('data-openfile')});
											if(that.pl_opener)
												that.reloadPlaylist().pl_opener = false;
											that.browseModal.parent().removeClass('active');
											return ;
										})
								)
