...
return this},updateSliderSize:function(){return this.durationSlider.css("width",function(){return $(this).parent().outerWidth()-$(this).parent().find("button.pull-left").outerWidth()-$(this).parent().find("button.pull-right").outerWidth()-42+"px"})},updateDuration:function(a){if(!this.durationInfo1)return this;this.durationInfo1.text((a*this.status.length).toString().toHHMMSS());this.durationInfo2.text(this.status.length.toString().toHHMMSS());return this},buildPlaylist:function(a){that=this;$.each(a.children,
function(){that.plModal.append('<li class="table-view-divider">'+this.name+"</li>");$.each(this.children,function(){that.plModal.append($("<li>",{"class":"table-view-cell media"}).addClass(this.current?"primary":"").append($('<a data-id="'+this.id+'" class="navigate-right" href=javascript:;>').append('<span class="badge">'+this.duration.toString().toHHMMSS()+"</span>").append($('<img class="media-object pull-left" src="/art?item='+this.id+'">').error(function(){$(this).attr("src","/img/vlc-48.png")})).append('<div class="media-body">'+
(this.current?'<span class="media-object pull-left icon icon-play"></span> ':"")+this.name+"</div>").click(function(){that.Play($(this).attr("data-id")).plModal.parent().parent().removeClass("active")})))})});return this},updateArt:function(){that=this;this.status.information?this.actualImg!=this.status.information.category.meta.filename&&this.albumArt.fadeOut(500,function(){meta=that.status.information.category.meta;$(this).removeAttr("height").removeAttr("width").removeClass("hidden").attr("src",
"/art?"+meta.filename).fadeIn(500).error(function(){$(this).attr("src","img/vlc-48.png")});h1=meta.artist&&meta.title?meta.artist+" - "+meta.title:meta.title?meta.title:"Informations";that.infoModal.find("header h1").html('<span class="icon icon-info"></span> '+h1).end().find(".content").empty().append($("<ul class=table-view>").append('<li class="table-view-divider">Meta</li>').append(meta.CONTENT_TYPE&&meta.genre?$("<li class=table-view-cell>").append(meta.CONTENT_TYPE).append("<p>"+meta.genre+
"</p>"):"").append(meta.CONTENT_TYPE&&!meta.genre?$("<li class=table-view-cell>").append("Type").append("<p>"+meta.CONTENT_TYPE+"</p>"):"").append(void 0!=meta.artist?$("<li class=table-view-cell>").append("Artist").append("<p>"+meta.artist+"</p>"):"").append(void 0!=meta.title?$("<li class=table-view-cell>").append("Title").append("<p>"+(0<meta.track_number&&0<meta.track_total?"["+meta.track_number+"/"+meta.track_total+"] - ":0<meta.track_number?meta.track_number+". ":"")+meta.title+"</p>"):"").append(void 0!=
meta.album&&"Film"!=meta.CONTENT_TYPE&&"TV Show"!=meta.CONTENT_TYPE?$("<li class=table-view-cell>").append("Album").append("<p>"+meta.album+"</p>"):"").append(meta.genre&&!meta.CONTENT_TYPE?$("<li class=table-view-cell>").append("Genre").append("<p>"+meta.genre+"</p>"):"").append(void 0!=meta.description?$("<li class=table-view-cell>").append("Summary").append("<p>"+nl2br(meta.description)+"</p>"):"").append(void 0!=meta.date?$("<li class=table-view-cell>").append("Release Date").append("<p>"+meta.date+
"</p>"):"").append(void 0!=meta.ACTOR?$("<li class=table-view-cell>").append("Actors").append("<p>"+meta.ACTOR+"</p>"):"").append(void 0!=meta.CHARACTER?$("<li class=table-view-cell>").append("Characters").append("<p>"+meta.CHARACTER+"</p>"):"").append(void 0!=meta.GUESTS?$("<li class=table-view-cell>").append("Guests").append("<p>"+meta.GUESTS+"</p>"):"").append(void 0!=meta.DIRECTOR?$("<li class=table-view-cell>").append("Director").append("<p>"+meta.DIRECTOR+"</p>"):"").append(void 0!=meta.WRITTEN_BY?
$("<li class=table-view-cell>").append("Written By").append("<p>"+meta.WRITTEN_BY+"</p>"):"").append(void 0!=meta.RECORDING_LOCATION?$("<li class=table-view-cell>").append("Recording Location").append("<p>"+meta.RECORDING_LOCATION+"</p>"):""));i=0;that.streamModal.find("ul.table-view").empty();for(isThereSubs=!1;c=that.status.information.category["Stream "+i];)"Audio"==c.Type?that.streamModal.find("ul.table-view").append($('<li class="table-view-cell">').append($('<span class="media-object pull-left icon icon-sound3">')).append("Audio "+
...
{"class":"control-item",html:"Playlist",href:"#playlist"}).on("click touch touchend",function(){$.ajax({url:server+"/requests/playlist.json",success:function(a){that.plModal.empty();that.buildPlaylist(a)},dataType:"json"})})).append($("<a>",{"class":"control-item",html:"Streams",href:"#streams"}))).appendTo("body");that.nSecFooter=$("<div>",{"class":"bar bar-standard bar-footer-secondary",css:{bottom:"50px"}}).append(that.durationInfo1=$("<button>",{type:"button","class":"btn pull-left control-item"}).on("click",
function(){that.Seek("-"+backwardDuration+"s")})).append(that.durationInfo2=$("<button>",{type:"button","class":"btn pull-right control-item"}).on("click",function(){that.Seek("+"+forwardDuration+"s")})).append(that.durationSlider=$('<input type="range" min="0" max="1" step=".001">').on("mouseup touchend",function(){that.Seek(Math.round($(this).val()*that.status.length))}).on("change",function(){that.updateDuration($(this).val())})).appendTo("body");that.refreshDuration();that.actualImg=meta.filename}):
(this.nSecondary&&(this.nSecondary.remove(),this.nSecFooter.remove()),this.albumArt.attr("src","img/vlc-48.png"));return this},error:function(a){0==a?""!=this.errorDiv.find("p").text()?this.errorDiv.empty().append($("<p>",{text:"Connection to VLC is re-established !"})).append($("<button>",{"class":"btn btn-positive",text:"Successful"})).fadeOut(2E3,function(){$(this).empty().fadeIn()}):this.errorDiv.empty():this.errorDiv.empty().append($("<p>",{text:a})).append($("<button>",{"class":"btn btn-negative",
text:"Reconnecting..."}));return this}};function nl2br(a,b){return(a+"").replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g,"$1"+(b||"undefined"===typeof b?"<br />":"<br>")+"$2")}String.prototype.toHHMMSS=function(){var a=parseInt(this,10),b=Math.floor(a/3600),d=Math.floor((a-3600*b)/60),a=a-3600*b-60*d;10>b&&(b="0"+b);10>d&&(d="0"+d);10>a&&(a="0"+a);return b+":"+d+":"+a};
