...
settings:d}}]).factory("VLC",["$http","SettingsService","$timeout",function(a,d,c){var k=d.settings.server,e=function(e,c){return a({method:"GET",url:e,params:c})};return{sendCommand:function(a){"object"!=typeof a&&(a={command:a});return e(k+"/requests/status.json",a)},Browse:function(a){return e(k+"/requests/browse.json",{uri:a})},Playlist:function(a){return e(k+"/requests/playlist.json","object"==typeof a?a:a?{command:"pl_empty"}:{})}}}]).filter("seconds",function(){return function(a){return(new Date(1970,
...
["$scope","VLC",function(a,d){this.load=function(c){return a.loadPlaylist(c)};this.Add=function(){l=1};this.Play=function(c,d){d&&f.forEach(d,function(a,f){d[f].current=a.id==c?!0:!1});a.sendCommand({command:"pl_play",id:c});return!0};this.Clear=function(){a.status.state&&"stopped"!=a.status.state&&!confirm("Something is currently playing and will be stopped. Would you continue ?")||this.load(!0)};this.Delete=function(a){this.load({command:"pl_delete",id:a})};this.Pl=function(c){a.sendCommand("pl_"+
c)}}]).controller("Browse",["$scope","VLC","SettingsService","$window",function(a,d,c,f){this.load=function(e){d.Browse(e).then(function(e){a.browse=e.data})};a.ordering=function(a){return".."==a.name?0:1};a.nullOrd=function(){return 1};this.load("file://"+f.encodeURIComponent(c.settings.browser));this.Play=function(e,c){if("s"==this.Ext(e))return a.sendCommand({command:"addsubtitle",val:c});a.sendCommand({command:"in_play",input:e});l--&&a.loadPlaylist()};this.Queue=function(e){l--?a.loadPlaylist({command:"in_enqueue",
input:e}):a.sendCommand({command:"in_enqueue",input:e})};this.Ext=function(a){a=a.substr(a.lastIndexOf(".")+1).toLowerCase();return-1<r.indexOf(a)?"v":-1<t.indexOf(a)?"a":-1<u.indexOf(a)?"p":-1<v.indexOf(a)?"s":!1};this.searchClick=function(){""!=this.search&&(this.search="")};this.isMRL=function(){return/:\/\//.test(this.search)};this.setDefault=function(a){a=f.decodeURIComponent(a).substr(7);confirm("Would you like to make this directory the default one ?\n\n"+a)&&(c.settings.browser=a)}}]).controller("Status",
["$scope","VLC","SettingsService","$filter","$interval","$location","$window",function(a,d,c,k,e,l,g){var p=this;a.loaded=!0;a.status={};a.error=!1;a.confs=c.settings;a.sendCommand=function(b){d.sendCommand(b).then(function(b){a.status=b.data;a.error=!1},function(b){a.error=b})};a.sendCommand({});a.focused=!0;var m,q=function(){if(f.isDefined(m))return!1;m=e(a.sendCommand,1E3*c.settings.update,0,!0,{})};g.addEventListener("blur",function(){e.cancel(m);m=void 0;a.focused=!1});g.addEventListener("focus",
function(){a.sendCommand({});q();a.focused=!0});q();a.closeModal=function(a){g.document.getElementById(a).classList.remove("active")};this.toggleFullscreen=function(){a.sendCommand("fullscreen")};this.Stop=function(){a.sendCommand("pl_stop")};this.PlayPause=function(){a.sendCommand("playing"==a.status.state?"pl_pause":"pl_play")};this.volIcon=function(b){switch(Math.floor(a.status.volume/2.56/25)){default:case 0:return b?"icon-sound3":"icon-sound4";case 1:case 2:return b?"icon-sound2":"icon-sound3";
case 3:case 4:case 5:case 6:case 7:case 8:return b?"icon-sound":"icon-sound2"}};this.Vol=function(b){return(256<=a.status.volume||230<a.status.volume)&&b&&a.confs.maxVol?a.sendCommand({command:"volume",val:256}):26>=a.status.volume&&!b?a.sendCommand({command:"volume",val:0}):a.sendCommand({command:"volume",val:b?a.status.volume+26:a.status.volume-26})};this.pos=new function(){this.timeout=null;this.pos=function(b){arguments.length&&(a.status.position=parseFloat(b),p.Seek(this.val));return a.status.position};
this.__defineGetter__("val",function(){return Math.round(this.pos()*this.len)});this.__defineGetter__("len",function(){return a.status.length})};this.Seek=function(b){!0===b&&(b="+"+c.settings.forward+"s");!1===b&&(b="-"+c.settings.backward+"s");if("undefined"==typeof b)return!1;a.sendCommand({command:"seek",val:b})};this.Chapter=function(b){var h=a.status.information.chapters,n=a.status.information.chapter;if(1<h.length&&c.settings.chapters)if(b){if(n<h.length)return a.sendCommand({command:"chapter",
val:n+1})}else if(0<n)return a.sendCommand({command:"chapter",val:n-1});return this.pos.val>c.settings.update&&0==b?this.Seek(0):this.Swipe(b)};this.Swipe=function(b){return a.sendCommand(b?"pl_next":"pl_previous")};this.info={keys:"CONTENT_TYPE title artist album genre description SEASON_SYNOPSIS SUMMARY date ACTOR CHARACTER GUESTS DIRECTOR PRODUCER WRITTEN_BY EXECUTIVE_PRODUCER DIRECTOR_OF_PHOTOGRAPHY SOUND_ENGINEER PRODUCTION_DESIGNER PRODUCTION_STUDIO RECORDING_LOCATION".split(" "),buildArr:function(){var b=
a.status;if(!b.information)return!1;var b=b.information.category.meta,h=[];f.forEach(this.keys,function(a,c){if(b[a]){if("CONTENT_TYPE"==a&&b.genre)return h.push({key:b[a],val:b.genre});if("album"!=a||"TV Show"!=b.CONTENT_TYPE&&"Film"!=b.CONTENT_TYPE)if("SUMMARY"!=a||"Film"!=b.CONTENT_TYPE)return"title"==a&&b.track_number?b.track_total?h.push({key:a,val:"["+b.track_number+"/"+b.track_total+"] "+b[a]}):h.push({key:a,val:b.track_number+". "+b[a]}):"date"==a?h.push({key:a,val:k("date")(b[a],"longDate")}):
h.push({key:a,val:b[a]})}});return this.infos=h},infos:[]};a.$watch("status",function(a,c){a.information&&c.information&&a.information.category.meta.filename==c.information.category.meta.filename||(p.info.buildArr(),p.streams.buildArr())});this.streams={buildArr:function(){var b=a.status;if(!b.information)return!1;b=b.information.category;this.streams=[];this.keyTranslations=[["T","Type","Type_","Typ","Tipo"],"C Codec Codec_ C\u00f3dec Codifica Kodek".split(" "),"L Language Langue_ Sprache Idioma Lingua Taal Linguagem Wyb\u00f3r_j\u0119zyka".split(" "),
"Ch Channels Canaux_ Canales Kan\u00e4le Canali Kanalen Canais Kana\u0142y".split(" "),["B","Bitrate","Tasa_de_bits","Przep\u0142ywno\u015b\u0107"]];this.TypeTranslations=[["V","Video","Vid\u00e9o","V\u00eddeo","Obraz"],["A","Audio","Audio","\u00c1udio","D\u017awi\u0119k"],"S Subtitle Sous-titres Subt\u00edtulo Untertitel Sottotitolo Ondertitel Legenda Napisy".split(" ")];this.findTranslation=function(a){var b=this.keyTranslations,c=this.TypeTranslations;f.forEach(a,function(d,e){for(var f=0;f<b.length;f++)if(-1!=
b[f].indexOf(e)){if("T"==b[f][0])for(var g=0;g<c.length;g++)-1!=c[g].indexOf(d)&&(a.TT=d,d=c[g][0]);a[b[f][0]]=d}},this);return a};f.forEach(b,function(a,b){var c=b.match(/[0-9]+$/);null!=c&&(this.streams[c[0]]=this.findTranslation(a));return!0},this)},streams:[],select:function(b){a.sendCommand({command:-1==b||"S"==this.streams[b].T?"subtitle_track":"audio_track",val:b})}};this.Rate=function(b){b=Math.round(100*a.status.rate+100*b)/100;if(.25>b||10<b)return!1;a.sendCommand({command:"rate",val:b})};
this.Delay=function(b,d){var e="a"==d?c.settings.audelay:c.settings.subdelay;a.sendCommand({command:"a"==d?"audiodelay":"subdelay",val:Math.round(1E3*(("a"==d?a.status.audiodelay:a.status.subtitledelay)+(b?e:-1*e)))/1E3})};this.EQ=new function(){f.element(g.document.getElementById("enableEq")).on("toggle",function(b){a.sendCommand({command:"enableeq",val:f.element(this).hasClass("active")?1:0})});var b=this;this.enabled=function(b){return!f.isUndefined(a.status.equalizer)&&f.isArray(a.status.equalizer)?
!1:"object"==typeof a.status.equalizer&&"undefined"!=typeof b?a.status.equalizer.hasOwnProperty(b):!0};this.timeout=null;this.setter=function(b,c){a.sendCommand("object"==typeof b?b:{command:b,val:c})};this.__defineGetter__("preamp",function(){return a.status.equalizer?this.enabled()?a.status.equalizer.preamp:0:!1});this.__defineSetter__("preamp",function(b){if(!this.enabled())return!1;a.status.equalizer.preamp=parseFloat(b);this.setter("preamp",this.preamp)});this.prsts=[];this.__defineGetter__("presets",
function(){if(!a.status.equalizer||!this.enabled())return!1;1>this.prsts.length&&a.status.equalizer.presets&&f.forEach(a.status.equalizer.presets,function(a,b){/[0-9]+/.test(b)&&this.prsts.push({id:b.match(/[0-9]+/)[0],val:a})},this);return this.prsts});this.__defineSetter__("presets",function(a){this.setter("setpreset",a)});this._b="60Hz 170Hz 310Hz 600Hz 1kHz 3kHz 6kHz 12kHz 14kHz 16kHz".split(" ");this.bands=[];f.forEach(this._b,function(c,d){this.bands.push({id:d,fq:c,val:function(c){if(!a.status.equalizer||
!b.enabled())return 0;arguments.length&&b.setter({command:"equalizer",band:d,val:c});return arguments.length?a.status.equalizer.bands['band id="'+d+'"']=c:a.status.equalizer.bands['band id="'+d+'"']}})},this)};a.loadPlaylist=function(b){d.Playlist(b).then(function(b){a.playlist=b.data.children})}}]).directive("resize",["$window",function(a){return function(d,c,k){var e=a.document.querySelector(".bar-footer-secondary .btn.control-item.pull-left"),l=a.document.querySelector(".bar-footer-secondary .btn.control-item.pull-right");
d.$watch(function(){return{w:a.innerWidth,l:e.clientWidth,r:l.clientWidth}},function(a,c){d.sliderSize=function(){return{width:a.w-46-a.r-a.l+"px"}}},!0);f.element(a).on("resize",function(){d.$apply()});f.element(a.document).ready(function(){d.$apply()})}}]).directive("fallbackSrc",function(){return{link:function(a,d,c){d.bind("error",function(){f.element(this).attr("src",c.fallbackSrc)})}}});var l=0,r="asf avi divx drc dv f4v flv gxf iso m1v m2v m2t m2ts m4v mkv mov mp2 mp4 mpeg mpeg1 mpeg2 mpeg4 mpg mts mtv mxf mxg nuv ogg ogm ogv ogx ps rec rm rmvb ts vob wmv xesc webm".split(" "),
